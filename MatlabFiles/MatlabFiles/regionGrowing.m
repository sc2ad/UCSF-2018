function [P, J] = regionGrowing(cIM, initPos, ffType, thresVal, maxDist, tfMean, tfFillHoles, tfSimplify)
...
%       ffType: 1 if using a standard fat fraction
%               2 if using a 10th of a percent fat fraction
...
%   Isabel Dominik - modifications June-Aug 2017
%   Susan Noworolski modified for ImProc II class Jan 2018
%
% Determine which threshold values to use
if ~exist('ffType', 'var')
    ffType = 1;
end
if ffType == 1
    lower_thresh = 0;   %if pixel is within these bounds, include in region
    upper_thresh = 45;
elseif ffType == 2
    lower_thresh = 0;	%ID had as 10, SMN changed to lower for 0.1% FF cases
    upper_thresh = 45; %ID had as 40, seems way too low (4%) for 0.1% FF cases
end

if exist('thresVal','var')
	upper_thresh = thresVal;
end
...
J(initPos(1), initPos(2), initPos(3)) = true;
    ...
    for j = -1:1
      for i = -1:1
        ...
        if xv+i > 0  &&  xv+i <= nRow &&...          % within the x-bounds?
           ...
           cIM(xv+i, yv+j, zv+k) <= upper_thresh &&... %(regVal + thresVal) &&...% within range
           cIM(xv+i, yv+j, zv+k) >= lower_thresh %(regVal - thresVal) % of the threshold?
